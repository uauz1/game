...
import { ArrowLeft, BookOpen, ChevronLeft, Trophy, Users, Sparkles, Brain, Camera, Search, Shuffle, Zap, UserRound, Puzzle, Gamepad2, Monitor, Settings as SettingsIcon } from 'lucide-react';
...
import { CharacterGuessGame, FamilyFeudGame, FastestGame, PhotoChallengeGame, RiddlesGame, WordBankGame } from './components/party/NewPartyGames';
...
const games = [
  ...
  {id:'photo',title:'تحدي الصورة',desc:'التفاصيل الصغيرة تخبّي الإجابة الكبيرة.',icon:Camera,tag:'تحدّي بصري',ready:true},
  {id:'words',title:'بنك الكلمات',desc:'اوصف الكلمة من غير ما تقول الكلمات الممنوعة.',icon:BookOpen,tag:'كلمات + وصف',ready:true},
  {id:'fast',title:'مين أسرع؟',desc:'لحظة واحدة تصنع الفارق.',icon:Zap,tag:'سرعة + زر',ready:true},
  {id:'character',title:'خمن الشخصية',desc:'تلميح وراء تلميح… من تكون؟',icon:UserRound,tag:'شخصيات',ready:true},
  {id:'riddles',title:'فوازير',desc:'فكّروا فيها سوا، الحل أقرب مما تتوقعون.',icon:Puzzle,tag:'ألغاز',ready:true},
  {id:'family',title:'تحدي العائلة',desc:'اكشفوا أشهر إجابات الجمهور واجمعوا النقاط.',icon:Trophy,tag:'إجابات جمهور',ready:true},
];
...
export default function App() {
  ...
  const go=(next:string)=>{if(games.some(game=>game.id===next)){setLastGame(next);try{localStorage.setItem('qaddha.last-game',next)}catch{/* Recent game is optional. */}}setHomeConfirm(false);setScreen(next); window.scrollTo({top:0,behavior:'instant'});};
  const isGame=games.some(game=>game.id===screen);
  const filteredGames=useMemo(()=>games.filter(game=>(gameFilter==='الكل'||gameGroups[game.id]===gameFilter)&&game.title.includes(gameSearch.trim())),[gameFilter,gameSearch]);
  const randomGame=()=>go(games[Math.floor(Math.random()*games.length)].id);
  ...
  return <div className="app party-app" dir="rtl"><header className="topbar"><button className="brand" aria-label="قدّها الرئيسية" onClick={()=>{if(screen==='home')window.scrollTo({top:0,behavior:'smooth'});else if(isGame)setHomeConfirm(true);else go('home');}} style={{padding:0,background:'transparent',border:0,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:'12px',minWidth:'190px'}}>
    ...
    {screen==='teams'?<TeamGame onHome={()=>go('home')}/>:screen==='letters'?<LettersGame onHome={()=>go('home')}/>:screen==='who'?<WhoAmIGame onHome={()=>go('home')}/>:screen==='photo'?<PhotoChallengeGame onHome={()=>setHomeConfirm(true)}/>:screen==='words'?<WordBankGame onHome={()=>setHomeConfirm(true)}/>:screen==='fast'?<FastestGame onHome={()=>setHomeConfirm(true)}/>:screen==='character'?<CharacterGuessGame onHome={()=>setHomeConfirm(true)}/>:screen==='riddles'?<RiddlesGame onHome={()=>setHomeConfirm(true)}/>:screen==='family'?<FamilyFeudGame onHome={()=>setHomeConfirm(true)}/>:<>
      ...
      <section id="games" className="lobby-games"><div className="section-head"><div><small>كل جمعة لها جوّها</small><h2>اختاروا التحدّي</h2></div><span>{games.length} ألعاب جاهزة الآن</span></div><div className="game-library-tools"><label><Search/><input aria-label="البحث في الألعاب" placeholder="ابحث عن لعبة…" value={gameSearch} onChange={event=>setGameSearch(event.target.value)}/></label><div>{['الكل','جماعية','سريعة','تحديات','كلمات','تخمين'].map(group=><button key={group} className={gameFilter===group?'active':''} onClick={()=>setGameFilter(group)}>{group}</button>)}</div><button className="random-game" onClick={randomGame}><Shuffle/> اختاروا لنا لعبة</button></div>{filteredGames.length?<div className="lobby-grid">{filteredGames.map((g)=>{const i=games.findIndex(game=>game.id===g.id);const Icon=g.icon;return <button key={g.id} className={`lobby-game game-${g.id} ${g.ready?'available':'upcoming'}`} disabled={!g.ready} onClick={()=>go(g.id)}><div className="game-art"><span className="game-number">{String(i+1).padStart(2,'0')}</span>{g.id==='teams'?<div className="mini-board">{[100,200,300,400,500,100].map((n,j)=><span key={j}>{n}</span>)}</div>:g.id==='letters'?<div className="mini-letters">{'ق د ه ا'.split(' ').map((l,j)=><span key={j}>{l}</span>)}</div>:<Icon className="cover-icon"/>}<span className="cover-spark">✦</span><span className="game-status">العب الآن</span></div><div className="game-copy"><small>{g.tag}</small><h3>{g.title}</h3><p>{g.desc}</p><span className="game-arrow"><ArrowLeft size={20}/></span></div></button>})}</div>:<div className="category-empty"><Search/><h3>ما لقينا لعبة بهذا الاسم</h3><p>امسح البحث أو اختر تصنيفًا ثانيًا.</p><button className="quiet" onClick={()=>{setGameSearch('');setGameFilter('الكل');}}>عرض كل الألعاب</button></div>}</section><section className="lobby-how"><Gamepad2/><div><h2>ثلاث خطوات… وتبدأ السالفة.</h2><p>كوّنوا فريقين، اختاروا تحدّيكم، وخلو واحد يمسك التقديم والتحكيم.</p></div><button className="secondary" onClick={()=>go('teams')}>جاهزين؟</button></section>
    </>}
  </main><footer><span>قدّها <b>✦</b></span><p>جمعتكم أحلى بالتحدّي</p></footer><SiteSettings open={settingsOpen} onClose={()=>setSettingsOpen(false)}/>{homeConfirm&&<div className="exit-overlay"><div role="dialog" aria-modal="true" aria-labelledby="site-home-confirm"><h2 id="site-home-confirm">نرجع للرئيسية؟</h2><p>إذا بدأت جولة، العودة الآن تنهيها وتحذف نقاطها.</p><button autoFocus className="primary" onClick={()=>setHomeConfirm(false)}>نكمل هنا</button><button className="quiet" onClick={()=>go('home')}>إنهاء والعودة للرئيسية</button></div></div>}</div>;
}
