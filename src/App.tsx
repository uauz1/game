...
import { loadCloudPlayerState, mergePlayerState, saveCloudPlayerState } from './utils/cloudPlayer';
import './arena.css';
import './newgames.css';
import './session.css';
...
const OnlineLobby = lazy(() => import('./components/party/OnlineLobby'));
...
export default function App() {
  ...
  const [onlineOpen,setOnlineOpen]=useState(Boolean(initialParams.get('online')));
  ...
  usePWA();
  const savePlayerData=(next:typeof playerData)=>{setPlayerData(next);try{localStorage.setItem(PLAYER_KEY,JSON.stringify(next))}catch{/* Guest history stays available for this visit. */}if(auth.session)void saveCloudPlayerState(next).catch(()=>undefined);};
  ...
  useEffect(()=>{
    ...
    let active=true;
    void loadCloudPlayerState().then(cloud=>{
      if(!active||!cloud)return;
      const local=readPlayerData();
      const merged=mergePlayerState(local,cloud);
      setPlayerData({favorites:merged.favorites,recent:merged.recent});
      try{localStorage.setItem(PLAYER_KEY,JSON.stringify(merged));}catch{/* optional */}
      void saveCloudPlayerState(merged).catch(()=>undefined);
    }).catch(()=>undefined);
    return()=>{active=false;};
  ...
  return <div className="app party-app" dir="rtl"><div className="tv-orientation-hint" role="status"><Cast aria-hidden="true"/><span><b>عرض التلفزيون جاهز</b> لفّ الجوال بالعرض ثم فعّل ملء الشاشة لأفضل نتيجة.</span></div><header className="topbar"><button className="brand" aria-label="قدّها الرئيسية" onClick={()=>{if(screen==='home')window.scrollTo({top:0,behavior:'smooth'});else if(isGame)requestHome();else go('home');}} style={{padding:0,background:'transparent',border:0,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:'12px',minWidth:'190px'}}>
    ...
    {screen==='teams'?<TeamGame onHome={requestHome}/>:screen==='letters'?<LettersGame onHome={requestHome}/>:screen==='who'?<WhoAmIGame onHome={requestHome}/>:screen==='photo'?<PhotoChallengeGame onHome={requestHome}/>:screen==='words'?<WordBankGame onHome={requestHome}/>:screen==='fast'?<FastestGame onHome={requestHome}/>:screen==='character'?<CharacterGuessGame onHome={requestHome}/>:screen==='riddles'?<RiddlesGame onHome={requestHome}/>:screen==='family'?<FamilyFeudGame onHome={requestHome}/>:screen==='connection'?<ConnectionGame onHome={requestHome}/>:screen==='auction'?<AuctionGame onHome={requestHome}/>:screen==='order'?<OrderGame onHome={requestHome}/>:screen==='memory'?<FlashMemoryGame onHome={requestHome}/>:screen==='missing'?<MissingGame onHome={requestHome}/>:screen==='acting'?<SilentActingGame onHome={requestHome}/>:screen==='secret'?<SecretWordGame onHome={requestHome}/>:screen==='pressure'?<PressureGame onHome={requestHome}/>:screen==='intruder'?<IntruderGame onHome={requestHome}/>:screen==='session'?<SmartPartySession games={games.map(({id,title,tag})=>({id,title,tag}))} onBack={()=>go('home')} onPlay={go}/>:<>
      <section className="lobby-hero"><div className="lobby-copy"><span className="eyebrow"><Sparkles size={15}/> افتحها… واجمعهم</span><h1>الجمعة عليكم.<br/><em>والتحدّي علينا.</em></h1><p>حوّلوا جلستكم إلى ساحة منافسة. فريقان، ضحكة، وسؤال يقلب الموازين… مين فيكم قدّها؟</p><div className="hero-actions"><button className="primary" onClick={()=>go('teams')}>ابدأ تحدّي الفرق <ChevronLeft size={19}/></button><button className="quiet smart-session-cta" onClick={()=>go('session')}><WandSparkles size={17}/> رتّب لنا جلسة</button><button className="quiet" onClick={()=>setOnlineOpen(true)}><Wifi size={17}/> لعب أونلاين</button><button className="quiet" onClick={()=>randomGame()}><Shuffle size={17}/> اختاروا لنا</button><a href="#games" className="quiet">شوف الألعاب <ArrowLeft size={17}/></a>{lastGame&&<button className="quiet recent-game" onClick={()=>go(lastGame)}>آخر لعبة: {games.find(game=>game.id===lastGame)?.title}</button>}</div><div className="hero-details"><span>اللعب المحلي بدون تسجيل</span><i/><span>للجوال والشاشة الكبيرة</span><i/><span>{games.length} لعبة جاهزة</span></div></div><div className="arena-illustration cinematic-arena" aria-hidden="true"><img src={asset('qaddha-majlis-hero.webp')} alt="" decoding="async" fetchPriority="high"/><span className="cinematic-sheen"/></div></section>
      <section className="quick-play-section" aria-label="تشغيل سريع"><div className="quick-play-heading"><span><Zap/> بدون إعدادات</span><h2>تشغيل سريع</h2><p>اختار الجو وابدأ مباشرة بضغطة واحدة.</p></div><div className="quick-play-grid"><button className="quick-online" onClick={()=>setOnlineOpen(true)}><Wifi/><span><b>مواجهة أونلاين</b><small>خصم حقيقي · حساب محفوظ · اتصال مباشر</small></span><ChevronLeft/></button><button onClick={()=>go('fast')}><Zap/><span><b>حماس سريع</b><small>مين أسرع؟ · مناسب لبداية الجلسة</small></span><ChevronLeft/></button><button onClick={()=>go('teams')}><Users/><span><b>فريق ضد فريق</b><small>قدّها فرق · منافسة كاملة</small></span><ChevronLeft/></button><button onClick={()=>go('connection')}><Brain/><span><b>تحدي ذكاء</b><small>وش الرابط؟ · تفكير وسرعة</small></span><ChevronLeft/></button><button onClick={()=>go('family')}><Trophy/><span><b>جو عائلي</b><small>تحدي العائلة · مقدم وفريقان</small></span><ChevronLeft/></button><button className="quick-session" onClick={()=>go('session')}><WandSparkles/><span><b>رتّب كل الجلسة</b><small>قدّها يختار عدة ألعاب ويضبطها لكم</small></span><ChevronLeft/></button>{lastGame&&<button className="quick-continue" onClick={()=>go(lastGame)}><Gamepad2/><span><b>ارجع لآخر لعبة</b><small>{games.find(game=>game.id===lastGame)?.title || 'آخر لعبة'}</small></span><ChevronLeft/></button>}</div></section>
