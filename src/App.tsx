...
import { ArrowLeft, BookOpen, Cast, ChevronLeft, Trophy, Users, Sparkles, Brain, Camera, Search, Shuffle, Zap, UserRound, Puzzle, Gamepad2, Heart, Link2, Monitor, Settings as SettingsIcon, WandSparkles } from 'lucide-react';
...
export default function App() {
  ...
  return <div className="app party-app" dir="rtl"><div className="tv-orientation-hint" role="status"><Cast aria-hidden="true"/><span><b>عرض التلفزيون جاهز</b> لفّ الجوال بالعرض ثم فعّل ملء الشاشة لأفضل نتيجة.</span></div><header className="topbar"><button className="brand" aria-label="قدّها الرئيسية" onClick={()=>{if(screen==='home')window.scrollTo({top:0,behavior:'smooth'});else if(isGame)setHomeConfirm(true);else go('home');}} style={{padding:0,background:'transparent',border:0,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:'12px',minWidth:'190px'}}>
    ...
    {screen==='teams'?<TeamGame onHome={()=>setHomeConfirm(true)}/>:screen==='letters'?<LettersGame onHome={()=>setHomeConfirm(true)}/>:screen==='who'?<WhoAmIGame onHome={()=>setHomeConfirm(true)}/>:screen==='photo'?<PhotoChallengeGame onHome={()=>setHomeConfirm(true)}/>:screen==='words'?<WordBankGame onHome={()=>setHomeConfirm(true)}/>:screen==='fast'?<FastestGame onHome={()=>setHomeConfirm(true)}/>:screen==='character'?<CharacterGuessGame onHome={()=>setHomeConfirm(true)}/>:screen==='riddles'?<RiddlesGame onHome={()=>setHomeConfirm(true)}/>:screen==='family'?<FamilyFeudGame onHome={()=>setHomeConfirm(true)}/>:screen==='connection'?<ConnectionGame onHome={()=>setHomeConfirm(true)}/>:screen==='auction'?<AuctionGame onHome={()=>setHomeConfirm(true)}/>:screen==='order'?<OrderGame onHome={()=>setHomeConfirm(true)}/>:screen==='memory'?<FlashMemoryGame onHome={()=>setHomeConfirm(true)}/>:screen==='missing'?<MissingGame onHome={()=>setHomeConfirm(true)}/>:screen==='party'?<SmartPartySession onHome={()=>go('home')}/>:screen==='help'?<HelpCenter onClose={()=>go('home')}/>:<section className="home-shell"><div className="hero"><div className="hero-copy"><p className="eyebrow">قدّها</p><h1>اللعبة جاهزة.<br/>والحماس عليكم.</h1><p>اختاروا لعبتكم، كوّنوا فرقكم، وخلو السهرة تمشي على كيفكم.</p><div className="hero-actions"><button className="primary-button" onClick={()=>randomGame(filteredGames)}><Shuffle size={18}/> لعب سريع</button>{lastGame&&games.some(game=>game.id===lastGame)&&<button className="secondary-button" onClick={()=>go(lastGame)}><ArrowLeft size={18}/> كمّل آخر لعبة</button>}<button className="secondary-button" onClick={()=>go('party')}><WandSparkles size={18}/> وضع الحفلة</button></div></div><div className="hero-card"><Gamepad2 size={54}/><span>اختاروا التحدّي</span><b>{games.length} لعبة</b></div></div><section className="game-toolbar"><div className="game-search"><Search size={18}/><input value={gameSearch} onChange={event=>setGameSearch(event.target.value)} placeholder="ابحث عن لعبة…" aria-label="ابحث عن لعبة"/></div><div className="game-filters">{['الكل','جماعية','كلمات','تخمين','تحديات','سريعة'].map(filter=><button key={filter} className={gameFilter===filter?'active':''} onClick={()=>setGameFilter(filter)}>{filter}</button>)}</div></section>{playerData.favorites.length>0&&<section className="favorites-strip"><div className="section-heading"><div><p>محفوظة عندك</p><h2>المفضلة</h2></div></div><div className="mini-game-row">{playerData.favorites.map(id=>games.find(game=>game.id===id)).filter(Boolean).map(game=>game&&<button key={game.id} onClick={()=>go(game.id)}><Heart size={16} fill="currentColor"/><span>{game.title}</span></button>)}</div></section>}<section className="games-section"><div className="section-heading"><div><p>اختاروا الليلة</p><h2>الألعاب</h2></div><span>{filteredGames.length} نتائج</span></div><div className="games-grid">{filteredGames.map(game=><article className="game-card" key={game.id}><button className="game-card-main" onClick={()=>go(game.id)} aria-label={`ابدأ ${game.title}`}><div className="game-cover"><img src={game.cover} alt="" loading="lazy"/><span className="game-tag">{game.tag}</span></div><div className="game-card-copy"><div className="game-title-row"><game.icon size={22}/><h3>{game.title}</h3></div><p>{game.desc}</p><span className="play-link">ابدأ الآن <ChevronLeft size={17}/></span></div></button><button className={`favorite-button ${playerData.favorites.includes(game.id)?'active':''}`} onClick={()=>toggleFavorite(game.id)} aria-label={playerData.favorites.includes(game.id)?`إزالة ${game.title} من المفضلة`:`إضافة ${game.title} للمفضلة`}><Heart size={18} fill={playerData.favorites.includes(game.id)?'currentColor':'none'}/></button></article>)}</div></section><section className="more-section"><div className="section-heading"><div><p>كل شيء بمكانه</p><h2>المزيد</h2></div></div><div className="more-grid"><button onClick={()=>setPlayerOpen(true)}><Trophy/><span>الإنجازات والسجل</span></button><button onClick={()=>setSettingsOpen(true)}><SettingsIcon/><span>الإعدادات</span></button><button onClick={()=>setHelpOpen(true)}><BookOpen/><span>المساعدة والتعليمات</span></button><button onClick={()=>go('party')}><WandSparkles/><span>وضع الحفلة</span></button></div></section></section>}
  </Suspense></main>{homeConfirm&&<div className="confirm-backdrop" role="presentation" onMouseDown={()=>setHomeConfirm(false)}><div className="confirm-dialog" role="dialog" aria-modal="true" aria-label="العودة للرئيسية" onMouseDown={event=>event.stopPropagation()}><h2>ترجع للرئيسية؟</h2><p>قد تفقد تقدم الجولة الحالية إذا لم تكن اللعبة تدعم الاستئناف.</p><div><button className="secondary-button" onClick={()=>setHomeConfirm(false)}>كمّل اللعب</button><button className="primary-button" onClick={()=>go('home')}>العودة للرئيسية</button></div></div></div>}<SiteSettings open={settingsOpen} onClose={()=>setSettingsOpen(false)}/><PlayerPanel open={playerOpen} onClose={()=>setPlayerOpen(false)} games={games.map(({id,title,cover})=>({id,title,cover}))} favorites={playerData.favorites} recent={playerData.recent} onPlay={id=>{setPlayerOpen(false);go(id);}} onOpenAuth={()=>{setPlayerOpen(false);setAuthOpen(true);}}/><Suspense fallback={null}>{authOpen&&<AuthModal onClose={()=>setAuthOpen(false)}/>} {helpOpen&&<HelpCenter onClose={()=>setHelpOpen(false)}/>}</Suspense></div>;
}
