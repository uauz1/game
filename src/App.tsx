...
export default function App() {
  ...
  </Suspense></main><footer><span>قدّها <b>✦</b></span><p>جمعتكم أحلى بالتحدّي</p></footer><SiteSettings open={settingsOpen} onClose={()=>setSettingsOpen(false)}/><PlayerPanel open={playerOpen} onClose={()=>setPlayerOpen(false)} games={games} favorites={playerData.favorites} recent={playerData.recent} onPlay={go} onToggleFavorite={toggleFavorite}/>{homeConfirm&&<div className="exit-overlay"><div role="dialog" aria-modal="true" aria-labelledby="site-home-confirm"><h2 id="site-home-confirm">نرجع للرئيسية؟</h2><p>إذا بدأت جولة، العودة الآن تنهيها وتحذف نقاطها.</p><button autoFocus className="primary" onClick={()=>setHomeConfirm(false)}>نكمل هنا</button><button className="quiet" onClick={()=>go('home')}>إنهاء والعودة للرئيسية</button></div></div>}</div>;
}
