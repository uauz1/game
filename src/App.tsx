...
const games = [
  ...
  {id:'who',title:'من أنا؟',desc:'شخصية غامضة وتلميحات تقرّبكم للحل.',icon:Brain,tag:'تخمين',ready:false},
...
export default function App() {
  ...
  return <div className="app party-app" dir="rtl"><header className="topbar"><button className="brand" aria-label="قدّها الرئيسية" onClick={()=>{if(screen==='home')window.scrollTo({top:0,behavior:'smooth'});}}><span className="brand-mark">ق</span><span>قدّها</span></button><span className="topbar-caption">للجمعة اللي تستاهل</span><span className="local-play"><Monitor size={16}/> شاشة واحدة · حماس الجميع</span></header><main>
    {screen==='teams'?<TeamGame onHome={()=>go('home')}/>:screen==='letters'?<LettersGame onHome={()=>go('home')}/>:<>
