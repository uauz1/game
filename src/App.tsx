...
const gameGroups: Record<string,string> = {teams:'جماعية',letters:'كلمات',who:'تخمين',photo:'تحديات',words:'كلمات',fast:'سريعة',character:'تخمين',riddles:'تحديات',family:'جماعية',connection:'تخمين'};
...
export default function App() {
  ...
  return <div className="app party-app" dir="rtl"><div className="tv-orientation-hint" role="status"><Cast aria-hidden="true"/><span><b>عرض التلفزيون جاهز</b> لفّ الجوال بالعرض ثم فعّل ملء الشاشة لأفضل نتيجة.</span></div><header className="topbar"><button className="brand" aria-label="قدّها الرئيسية" onClick={()=>{if(screen==='home')window.scrollTo({top:0,behavior:'smooth'});else if(isGame)setHomeConfirm(true);else go('home');}} style={{padding:0,background:'transparent',border:0,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:'12px',minWidth:'190px'}}>
    ...
    {screen==='teams'?<TeamGame onHome={()=>setHomeConfirm(true)}/>:screen==='letters'?<LettersGame onHome={()=>setHomeConfirm(true)}/>:screen==='who'?<WhoAmIGame onHome={()=>setHomeConfirm(true)}/>:screen==='photo'?<PhotoChallengeGame onHome={()=>setHomeConfirm(true)}/>:screen==='words'?<WordBankGame onHome={()=>setHomeConfirm(true)}/>:screen==='fast'?<FastestGame onHome={()=>setHomeConfirm(true)}/>:screen==='character'?<CharacterGuessGame onHome={()=>setHomeConfirm(true)}/>:screen==='riddles'?<RiddlesGame onHome={()=>setHomeConfirm(true)}/>:screen==='family'?<FamilyFeudGame onHome={()=>setHomeConfirm(true)}/>:screen==='connection'?<ConnectionGame onHome={()=>setHomeConfirm(true)}/>:screen==='session'?<SmartPartySession games={games.map(({id,title,tag})=>({id,title,tag}))} onBack={()=>go('home')} onPlay={go}/>:<>
