  ...
  return <main dir="rtl" className="min-h-screen bg-[#070707] text-white pb-28">
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 space-y-5">
      ...
      <section className="rounded-[28px] border border-white/10 bg-[#111] p-5">
        ...
        {(overview?.recent_rooms||[]).length?<div className="overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead className="text-zinc-500"><tr className="border-b border-white/10"><th className="py-3 text-right">الكود</th><th className="text-right">اللعبة</th><th className="text-right">النوع</th><th className="text-right">الحالة</th><th className="text-right">اللاعبون</th><th className="text-right">آخر تحديث</th></tr></thead><tbody>{overview?.recent_rooms.map(room=><tr key={room.id} className="border-b border-white/5"><td className="py-3 font-mono font-black text-amber-300">{room.code}</td><td>{GAME_NAMES[room.game_id]||room.game_id}</td><td>{room.mode==='quick'?'مطابقة سريعة':'خاصة'}</td><td><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${room.status==='playing'?'bg-emerald-500/10 text-emerald-300':room.status==='ready'?'bg-blue-500/10 text-blue-300':room.status==='waiting'?'bg-amber-500/10 text-amber-300':'bg-zinc-800 text-zinc-400'}`}>{room.status}</span></td><td>{room.members}</td><td className="text-zinc-500">{timeAgo(room.updated_at)}</td></tr>)}</tbody></table></div>:<div className="rounded-xl border border-dashed border-white/10 p-6 text-center text-sm text-zinc-500">ما فيه غرف أونلاين محفوظة إلى الآن.</div>}
      </section>
