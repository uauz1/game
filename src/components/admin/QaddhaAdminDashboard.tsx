...
export default function QaddhaAdminDashboard(){
  ...
  return <main dir="rtl" className="min-h-screen bg-[#070707] text-white pb-28">
    <header className="sticky top-0 z-40 border-b border-white/10 bg-black/85 backdrop-blur-xl"><div className="mx-auto max-w-7xl px-4 py-4 flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-amber-300">CONTROL CENTER</p><h1 className="text-xl sm:text-2xl font-black">لوحة تحكم قدّها</h1></div><div className="flex gap-2"><a href={siteBaseUrl} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm font-bold">فتح الموقع</a><button onClick={runHealth} className="rounded-xl bg-amber-400 px-3 py-2 text-sm font-black text-black">فحص شامل</button></div></div></header>
    <div className="mx-auto max-w-7xl px-4 py-6 space-y-5">
      ...
      <section className="rounded-[28px] border border-white/10 bg-[#111] p-5"><div className="flex items-center gap-3 mb-4"><Settings2 className="text-amber-300"/><h3 className="font-black">أدوات سريعة</h3></div><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-2"><a href={siteBaseUrl} target="_blank" className="rounded-2xl border border-white/10 bg-black/30 p-4 font-bold">فتح قدّها</a><a href="https://github.com/uauz1/game" target="_blank" rel="noreferrer" className="rounded-2xl border border-white/10 bg-black/30 p-4 font-bold">GitHub</a><a href="https://supabase.com/dashboard/project/uhbtcjlapgpsohbkotpd" target="_blank" rel="noreferrer" className="rounded-2xl border border-white/10 bg-black/30 p-4 font-bold">Supabase</a><button onClick={runHealth} className="rounded-2xl border border-white/10 bg-black/30 p-4 text-right font-bold"><Activity className="inline ml-2 text-amber-300" size={17}/>تشغيل الفحص</button></div></section>
    </div>
  </main>;
}
