...
type Room = {
  ...
  pausedAt: number | null; answerRevealed: boolean; winner: 0 | 1 | null; gameActions: OnlineGameAction[];
};
type ClientMessage =
...
const newRoom = (code: string, name: string, gameId: string): Room => ({
  ...
  pausedAt: null, answerRevealed: false, winner: null, gameActions: [],
});
const normalize = (room: Room): Room => ({ ...room, teamNames: room.teamNames || ['الفريق الأول', 'الفريق الثاني'], timerSeconds: room.timerSeconds || 60, difficulty: room.difficulty || 'mixed', category: room.category || 'الكل', maxPlayers: room.maxPlayers || MAX_PLAYERS, pausedAt: room.pausedAt || null, answerRevealed: room.answerRevealed || false, gameActions: Array.isArray(room.gameActions) ? room.gameActions.slice(-120) : [] });
...
export default function OnlineRoom({ games, onBack }: { games: GameOption[]; onBack: () => void }) {
  ...
  const teamsReady = connectedPlayers.some(p => p.team === 0) && connectedPlayers.some(p => p.team === 1);
  const allReady = !!room && connectedPlayers.length >= 2 && teamsReady && connectedPlayers.every(p => p.host || p.ready);
