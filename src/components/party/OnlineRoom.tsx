...
const timerOptionsForGame = (gameId: string) => {
  ...
  if (gameId === 'pressure') return [15, 20, 30];
  if (['who','acting'].includes(gameId)) return [30, 45, 60];
  if (gameId === 'intruder') return [30];
  return [20, 30, 45, 60];
};
...
export default function OnlineRoom({ games, onBack, onRequireAuth, accountName }: { games: GameOption[]; onBack: () => void; onRequireAuth?: () => void; accountName?: string }) {
    ...
    void getAuthClient().then(instance => {
      ...
      channel.on('broadcast', { event: 'client-message' }, message => {
        ...
        if (incoming.type === 'join') {
          ...
          const normalizedAction = normalizeGameAction(incoming.action, incoming.playerId, current.gameId);
          if (!normalizedAction) return;
          const action = authorizeGameAction(normalizedAction);
          const frame = document.getElementById('qaddha-online-game-frame') as HTMLIFrameElement | null;
          frame?.contentWindow?.postMessage({ type: 'qaddha-online-replay', action, authoritative: true }, window.location.origin);
  ...
  useEffect(() => {
    ...
    const receiveEmbeddedAction = (event: MessageEvent) => {
      ...
      if (!channel) return;
      if (mode === 'host') {
        const authoritativeAction = authorizeGameAction(action);
        rememberGameAction(authoritativeAction);
        void channel.send({ type: 'broadcast', event: 'server-message', payload: { type: 'game-action', action: authoritativeAction } });
      } else if (mode === 'guest') {
  ...
  useEffect(() => {
    ...
    document.documentElement.classList.toggle('qaddha-online-playing', playing);
    document.body.classList.toggle('qaddha-online-playing', playing);
    return () => {
      document.documentElement.classList.remove('qaddha-online-playing');
      document.body.classList.remove('qaddha-online-playing');
    };
  }, [room?.phase]);
    ...
    const tryLaunch = () => {
      ...
      const allLoaded = activePlayers.length >= minimumPlayers && activePlayers.every(player => player.gameLoadedId === current.gameId);
  ...
  return <section className={`online-room ${room.phase === 'playing' ? 'online-room-playing' : ''}`} dir="rtl"><Header room={room} connected={connected} host={mode === 'host'} onBack={exitOnline}/>{notice && <p className="online-notice" role="status">{notice}</p>}
    {room.phase === 'lobby' ? <div className="online-lobby-layout"><main className="online-lobby-main"><div className="online-code-card"><div><small>كود الغرفة</small><strong dir="ltr">{room.code}</strong><span>{room.players.filter(p => p.connected).length} من {room.maxPlayers} متصلين · الحد الأدنى {minimumPlayers}</span></div><div className="online-share-actions"><button onClick={copyCode}>{copied ? <Check/> : <Copy/>}{copied ? 'تم النسخ' : 'نسخ الكود'}</button><button onClick={copy}><Link2/> نسخ الرابط</button><button onClick={share}><Share2/> مشاركة</button></div></div>{teamsRequired ? <TeamBoard room={room} canRemove={mode === 'host'} onRemove={remove}/> : <IndividualBoard room={room} canRemove={mode === 'host'} onRemove={remove}/>}<div className={`online-ready-summary ${allReady && connected ? 'ready' : ''}`}><div><Check/><span>الجاهزية</span><strong>{connectedPlayers.filter(player => player.host || player.ready).length}/{connectedPlayers.length || minimumPlayers}</strong></div><div><Wifi/><span>الاتصال</span><strong>{connected ? 'مستقر' : 'يعاد الاتصال'}</strong></div><div><Users/><span>المطلوب</span><strong>{minimumPlayers}+</strong></div><p>{!connected ? 'ننتظر رجوع الاتصال قبل البداية.' : connectedPlayers.length < minimumPlayers ? `نحتاج ${minimumPlayers - connectedPlayers.length} لاعب إضافي.` : !teamsReady ? 'وزّعوا اللاعبين على الفريقين.' : !connectedPlayers.every(player => player.host || player.ready) ? 'بانتظار جاهزية بقية اللاعبين.' : 'الغرفة جاهزة للانطلاق.'}</p></div> {mode === 'guest' && localPlayer && <div className="guest-controls">{teamsRequired && <><button disabled={!connected} className={localPlayer.team === 0 ? 'active' : ''} onClick={() => send({ type: 'team', playerId: me, team: 0 })}>{room.teamNames[0]}</button><button disabled={!connected} className={localPlayer.team === 1 ? 'active' : ''} onClick={() => send({ type: 'team', playerId: me, team: 1 })}>{room.teamNames[1]}</button></>}<button disabled={!connected} className={`ready ${localPlayer.ready ? 'active' : ''}`} onClick={() => send({ type: 'ready', playerId: me, ready: !localPlayer.ready })}>{localPlayer.ready ? <><Check/> جاهز</> : 'أعلن جاهزيتي'}</button></div>}{mode === 'host' && <div className="host-lobby-controls"><div className="host-settings-title"><Settings2/><div><b>إعداد المباراة</b><small>كل الخيارات في مكان واحد</small></div></div><label><span>اللعبة</span><select value={room.gameId} onChange={e => update(r => {
