...
export function SecretWordGame({ onHome }: Props) {
  const [names,setNames] = useState(['لاعب 1','لاعب 2','لاعب 3','لاعب 4']);
  const [duration,setDuration] = useState(120);
  const [phase,setPhase] = useState<'setup'|'handoff'|'role'|'talk'|'vote'|'result'>('setup');
  const [index,setIndex] = useState(0);
  const [spy,setSpy] = useState(0);
  const [set,setSet] = useState(spySets[0]);
  const [votes,setVotes] = useState<number[]>([]);
  const start = () => { const clean=names.map((name,i)=>name.trim()||`لاعب ${i+1}`); setNames(clean); setSpy(Math.floor(Math.random()*clean.length)); setSet(spySets[Math.floor(Math.random()*spySets.length)]); setIndex(0); setVotes([]); setPhase('handoff'); };
  const nextPlayer = () => { if(index+1>=names.length)setPhase('talk'); else { setIndex(value=>value+1); setPhase('handoff'); } };
