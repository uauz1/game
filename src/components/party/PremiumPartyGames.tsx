...
export function PressureGame({ onHome }: { onHome: () => void }) {
  ...
  const startGame=()=>{
    ...
    setDeck(next);setRound(0);setTurn(0);setStarted(false);setTimeLeft(next[0].seconds);setTeams(value=>value.map(team=>({...team,score:0})));setPhase('playing');
  };
