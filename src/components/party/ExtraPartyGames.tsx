...
const ONLINE_ROUNDS = Number(ONLINE_PARAMS.get('onlineRounds') || '0');
const onlineRoundLimit = (fallback:number) => ONLINE_EMBED && [4,6,8].includes(ONLINE_ROUNDS) ? Math.min(ONLINE_ROUNDS, fallback) : fallback;
...
export function AuctionGame({ onHome }:{onHome:()=>void}) {
  ...
  const totalRounds=onlineRoundLimit(auctionRounds.length); const current=auctionRounds[round%totalRounds];
  ...
  const next=()=>{if(round+1>=totalRounds){setFinished(true);return;}setRound(r=>r+1);setBid(3);setTeam(t=>1-t);setPicked([]);setDone(false)};
  ...
  return <Shell title="المزاد" subtitle="زايدوا على عدد الإجابات… واللي يرسو عليه المزاد لازم يثبت كلامه." icon={<Gavel/>} onHome={onHome}>
    {finished?<Finished score={`${score[0]} - ${score[1]}`} total={totalRounds} onRestart={restart}/>:<><div className="extra-score"><b>{ONLINE_TEAM_NAMES[0]} <span>{score[0]}</span></b><b>الجولة {round+1} / {totalRounds}</b><b>{ONLINE_TEAM_NAMES[1]} <span>{score[1]}</span></b></div>
...
export function OrderGame({ onHome }:{onHome:()=>void}) {
  const [round,setRound]=useState(0); const totalRounds=onlineRoundLimit(orderRounds.length); const current=orderRounds[round%totalRounds]; const [items,setItems]=useState(()=>shuffle(current.items)); const [score,setScore]=useState(0); const [state,setState]=useState<'play'|'right'|'wrong'>('play'); const [finished,setFinished]=useState(false);
  ...
  const next=()=>{if(round+1>=totalRounds){setFinished(true);return;}const r=round+1;setRound(r);setItems(shuffle(orderRounds[r].items));setState('play')};
  const restart=()=>{setRound(0);setItems(shuffle(orderRounds[0].items));setScore(0);setState('play');setFinished(false)};
  return <Shell title="رتّبها" subtitle="حط الأشياء بالترتيب الصحيح قبل ما يضحك عليكم الفريق الثاني." icon={<Sparkles/>} onHome={onHome}>{finished?<Finished score={score} total={totalRounds} onRestart={restart}/>:<><div className="extra-score single"><b>النقاط <span>{score}</span></b><b>الجولة {round+1} / {totalRounds}</b></div><article className="extra-card"><h2>{current.title}</h2><div className="order-list">{items.map((x,i)=><div key={x}><b>{i+1}</b><span>{x}</span><button aria-label="تحريك لأعلى" onClick={()=>move(i,-1)}><ChevronUp/></button><button aria-label="تحريك لأسفل" onClick={()=>move(i,1)}><ChevronDown/></button></div>)}</div>{state==='play'?<button className="primary extra-submit" onClick={check}>تحقق من الترتيب</button>:<div className={`extra-result ${state}`} >{state==='right'?<Check/>:<X/>}<b>{state==='right'?'ترتيب صحيح!':'مو كذا… الترتيب الصحيح:'}</b>{state==='wrong'&&<p>{current.items.join(' ← ')}</p>}<button className="secondary" onClick={next}>{round+1===orderRounds.length?'عرض النتيجة':'الجولة التالية'}</button></div>}</article></>}</Shell>
}

export function FlashMemoryGame({ onHome }:{onHome:()=>void}) {
  const [round,setRound]=useState(0); const totalRounds=onlineRoundLimit(memoryRounds.length); const current=memoryRounds[round%totalRounds]; const [phase,setPhase]=useState<'show'|'answer'|'done'>('show'); const [chosen,setChosen]=useState<string[]>([]); const [score,setScore]=useState(0); const [finished,setFinished]=useState(false);
