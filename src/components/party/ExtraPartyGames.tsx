...
export function AuctionGame({ onHome }:{onHome:()=>void}) {
  ...
  useEffect(()=>{if(!finished&&!done){publishMultiplayerChallenge({gameId:'auction',roundKey:`${current.id}-${bid}-${team}`,answers:[],choices:options,mode:'multi',multiAnswers:current.answers,requiredSelections:bid,eligibleTeam:team===1?1:0,points:1});return()=>clearMultiplayerChallenge('auction');}clearMultiplayerChallenge('auction');},[bid,current,done,finished,options,team]);
  ...
  useEffect(()=>{const receive=(event:Event)=>{const detail=(event as CustomEvent<{gameId?:string;team?:number}>).detail;if(detail?.gameId!=='auction'||finished||done)return;const scoringTeam=detail.team===1?1:0;if(scoringTeam!==team)return;setScore(value=>value.map((item,index)=>index===scoringTeam?item+1:item) as [number,number]);setPicked(current.answers.slice(0,bid));setDone(true);};window.addEventListener('qaddha:multiplayer-team-score',receive);return()=>window.removeEventListener('qaddha:multiplayer-team-score',receive);},[bid,current.answers,done,finished,team]);
...
export function OrderGame({ onHome }:{onHome:()=>void}) {
  ...
  const phoneChoices=useMemo(()=>shuffle(current.items),[current]);
  useEffect(()=>{publishMultiplayerTeamNames(names);},[names]);
  useEffect(()=>{if(!finished&&state==='play'){publishMultiplayerChallenge({gameId:'order',roundKey:current.id,answers:[],choices:phoneChoices,mode:'sequence',sequenceAnswers:current.items,points:1});return()=>clearMultiplayerChallenge('order');}clearMultiplayerChallenge('order');},[current,finished,phoneChoices,state]);
  ...
  useEffect(()=>{const receive=(event:Event)=>{const detail=(event as CustomEvent<{gameId?:string;team?:number}>).detail;if(detail?.gameId!=='order'||finished||state!=='play')return;const team=detail.team===1?1:0;setScore(value=>value.map((item,index)=>index===team?item+1:item) as [number,number]);setItems([...current.items]);setState('right');};window.addEventListener('qaddha:multiplayer-team-score',receive);return()=>window.removeEventListener('qaddha:multiplayer-team-score',receive);},[current.items,finished,state]);
  const result=score[0]===score[1]?`تعادل ${score[0]} - ${score[1]}`:`${score[0]>score[1]?names[0]:names[1]} فاز · ${score[0]} - ${score[1]}`;
...
export function FlashMemoryGame({ onHome }:{onHome:()=>void}) {
  ...
  useEffect(()=>{if(!finished&&phase==='answer'){publishMultiplayerChallenge({gameId:'memory',roundKey:current.id,answers:[],choices:pool,mode:'sequence',sequenceAnswers:current.items,points:1});return()=>clearMultiplayerChallenge('memory');}clearMultiplayerChallenge('memory');},[current,finished,phase,pool]);
  ...
  useEffect(()=>{const receive=(event:Event)=>{const detail=(event as CustomEvent<{gameId?:string;team?:number}>).detail;if(detail?.gameId!=='memory'||finished||phase!=='answer')return;const team=detail.team===1?1:0;setScore(value=>value.map((item,index)=>index===team?item+1:item) as [number,number]);setChosen([...current.items]);setPhase('done');};window.addEventListener('qaddha:multiplayer-team-score',receive);return()=>window.removeEventListener('qaddha:multiplayer-team-score',receive);},[current.items,finished,phase]);
