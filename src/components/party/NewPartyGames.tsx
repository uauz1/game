...
export function CharacterGuessGame({ onHome }: GameProps) {
  ...
  useEffect(()=>{if(phase==='play'&&card&&!choice){publishMultiplayerChallenge({gameId:'character',roundKey:card.id,answers:[card.answer],choices:card.options,points:(4-hint)*100});return()=>clearMultiplayerChallenge('character');}clearMultiplayerChallenge('character');},[card,choice,hint,phase]);
  ...
  useEffect(()=>{const receive=(event:Event)=>{const detail=(event as CustomEvent<{gameId?:string;team?:number;points?:number}>).detail;if(detail?.gameId!=='character'||phase!=='play'||choice)return;const team=detail.team===1?1:0;const points=typeof detail.points==='number'?detail.points:(4-hint)*100;setTeams(value=>value.map((item,index)=>index===team?{...item,score:item.score+points}:item));setChoice(card.answer);};window.addEventListener('qaddha:multiplayer-team-score',receive);return()=>window.removeEventListener('qaddha:multiplayer-team-score',receive);},[card.answer,choice,hint,phase]);
...
export function RiddlesGame({ onHome }: GameProps) {
  ...
  useEffect(()=>{const receive=(event:Event)=>{const detail=(event as CustomEvent<{gameId?:string;team?:number;points?:number}>).detail;if(detail?.gameId!=='riddles'||phase!=='play'||revealed||timedOut)return;const team=detail.team===1?1:0;const points=typeof detail.points==='number'?detail.points:100;setTeams(value=>value.map((item,index)=>index===team?{...item,score:item.score+points}:item));if(round+1>=deck.length)setPhase('result');else{setRound(value=>value+1);setRevealed(false);setTimedOut(false);}};window.addEventListener('qaddha:multiplayer-team-score',receive);return()=>window.removeEventListener('qaddha:multiplayer-team-score',receive);},[deck.length,phase,revealed,round,timedOut]);
...
export function PhotoChallengeGame({ onHome }: GameProps) {
  ...
  useEffect(()=>{if(phase==='play'&&card&&!revealed&&!timedOut){publishMultiplayerChallenge({gameId:'photo',roundKey:card.id,answers:[card.answer],points:(3-level)*100});return()=>clearMultiplayerChallenge('photo');}clearMultiplayerChallenge('photo');},[card,level,phase,revealed,timedOut]);
  useEffect(()=>{const receive=(event:Event)=>{const detail=(event as CustomEvent<{gameId?:string;team?:number;points?:number}>).detail;if(detail?.gameId!=='photo'||phase!=='play'||revealed||timedOut)return;const team=detail.team===1?1:0;const points=typeof detail.points==='number'?detail.points:(3-level)*100;setTeams(value=>value.map((item,index)=>index===team?{...item,score:item.score+points}:item));setRevealed(true);};window.addEventListener('qaddha:multiplayer-team-score',receive);return()=>window.removeEventListener('qaddha:multiplayer-team-score',receive);},[level,phase,revealed,timedOut]);
...
export function FastestGame({ onHome }: GameProps) {
  ...
  useEffect(()=>{const receive=(event:Event)=>{const detail=(event as CustomEvent<{gameId?:string;team?:number}>).detail;if(detail?.gameId!=='fast'||phase!=='play'||expired||revealed||buzz!==null)return;buzzer(detail.team===1?1:0);};window.addEventListener('qaddha:multiplayer-buzz',receive);return()=>window.removeEventListener('qaddha:multiplayer-buzz',receive);},[buzz,expired,phase,revealed]);
  useEffect(()=>{const receive=(event:Event)=>{const detail=(event as CustomEvent<{gameId?:string;team?:number}>).detail;if(detail?.gameId!=='fast'||phase!=='play'||expired||revealed||buzz===null)return;const losing=detail.team===1?1:0;if(losing===buzz)setBuzz(1-losing);};window.addEventListener('qaddha:multiplayer-wrong',receive);return()=>window.removeEventListener('qaddha:multiplayer-wrong',receive);},[buzz,expired,phase,revealed]);
  ...
  useEffect(()=>{const receive=(event:Event)=>{const detail=(event as CustomEvent<{gameId?:string;team?:number;points?:number}>).detail;if(detail?.gameId!=='fast'||phase!=='play'||expired||revealed)return;const team=detail.team===1?1:0;const points=typeof detail.points==='number'?detail.points:100;setTeams(value=>value.map((item,index)=>index===team?{...item,score:item.score+points}:item));next();};window.addEventListener('qaddha:multiplayer-team-score',receive);return()=>window.removeEventListener('qaddha:multiplayer-team-score',receive);},[deck.length,expired,phase,revealed,round]);
...
export function ConnectionGame({ onHome }: GameProps) {
  ...
  useEffect(()=>{if(phase==='play'&&card&&!revealed&&!timedOut){publishMultiplayerChallenge({gameId:'connection',roundKey:card.id,answers:[card.answer,...card.aliases],points});return()=>clearMultiplayerChallenge('connection');}clearMultiplayerChallenge('connection');},[card,phase,points,revealed,timedOut]);
  ...
  useEffect(()=>{const receive=(event:Event)=>{const detail=(event as CustomEvent<{gameId?:string;team?:number;points?:number}>).detail;if(detail?.gameId!=='connection'||phase!=='play'||revealed||timedOut)return;const team=detail.team===1?1:0;const awarded=typeof detail.points==='number'?detail.points:points;setTeams(value=>value.map((item,index)=>index===team?{...item,score:item.score+awarded}:item));setNotice(`إجابة صحيحة من الجوال · +${awarded} نقطة`);setRevealed(true);};window.addEventListener('qaddha:multiplayer-team-score',receive);return()=>window.removeEventListener('qaddha:multiplayer-team-score',receive);},[phase,points,revealed,timedOut]);
