...
export default function SecretWordPrivate({ onHome }: { onHome: () => void }) {
    ...
    channel
      .on('broadcast', { event: 'join' }, ({ payload }) => {
        ...
        setPlayers(current => {
          ...
          if (!exists && current.length >= 10) return current;
          return exists ? current.map(item => item.id === id ? { ...item, name } : item) : [...current, { id, name }];
        });
        window.setTimeout(() => syncOne(id), 80);
      })
      .on('broadcast', { event: 'vote' }, ({ payload }) => {
        if (!payload || typeof payload !== 'object') return;
        const message = payload as Partial<ClientMessage>;
        if (message.type !== 'vote' || typeof message.playerId !== 'string' || typeof message.targetId !== 'string') return;
        setVotes(current => current[message.playerId!] ? current : { ...current, [message.playerId!]: message.targetId! });
      })
  ...
  useEffect(() => { if (phase === 'vote' && players.length >= 3 && Object.keys(votes).length >= players.length) setPhase('result'); }, [phase, players.length, votes]);
  ...
  const start = () => {
    ...
    players.forEach((player, index) => { nextRoles[player.id] = player.id === nextSpy ? { spy: true } : { spy: false, place: nextSet.place, clue: nextSet.words[index % nextSet.words.length] }; });
  ...
  return <section className="final-game secret-game" dir="rtl"><header className="final-head"><button className="quiet" onClick={onHome}>الألعاب</button><div><span><Sparkles/></span><small>سرّية متعددة الجوالات</small><h1>الكلمة السرّية</h1><p>كل لاعب يستلم دوره على جواله، ولا أحد يحتاج يمرر جهازه للثاني.</p></div></header>
    {phase === 'lobby' ? <article className="final-panel setup-panel"><h2>ادخلوا الغرفة من جوالاتكم</h2><p>امسحوا نفس QR من كل جوال، واكتبوا أسماءكم. الاتصال الآن عبر خادم قدّها ويعمل حتى مع شبكات مختلفة.</p><section className="host-pairing"><div className="host-pairing-copy"><span><Smartphone/> غرفة اللاعبين</span><h3>{players.length} لاعبين متصلين</h3><div className={`host-connection ${players.length >= 3 ? 'connected' : ''}`}>{players.length >= 3 ? <Wifi/> : <RefreshCw className={status === 'starting' || status === 'ready' ? 'spin' : ''}/>}<b>{status === 'error' ? 'تعذر الاتصال بالخادم' : players.length >= 3 ? 'جاهزين للبدء' : 'بانتظار 3 لاعبين على الأقل'}</b></div><button className="quiet host-copy" disabled={!hostUrl} onClick={copy}>{copied ? <Check/> : <Copy/>}{copied ? 'تم نسخ الرابط' : 'نسخ رابط اللاعبين'}</button></div><div className="host-qr">{qrCode ? <img src={qrCode} alt="QR دخول لعبة الكلمة السرية"/> : <div className="qr-loading"><RefreshCw className="spin"/><span>نجهز الغرفة…</span></div>}</div></section><div className="player-names" style={{ marginTop: 18 }}>{players.map(player => <div key={player.id} style={{display:'flex',alignItems:'center',gap:10,padding:'12px 14px',border:'1px solid #ffffff18',borderRadius:14}}><Check size={17}/><b>{player.name}</b><small style={{marginInlineStart:'auto'}}>متصل</small></div>)}</div><button className="primary" disabled={players.length < 3} onClick={start}>وزّع الأدوار وابدأ <Users/></button></article>
...
export function SecretWordPhone({ roomId, token }: { roomId: string; token: string }) {
  ...
  const playerIdRef = useRef(`p-${crypto.randomUUID().replace(/-/g,'').slice(0,20)}`);
  const channelRef = useRef<ReturnType<typeof createRealtimeRoomChannel> | null>(null);
  ...
  useEffect(() => {
    ...
    const channel = createRealtimeRoomChannel('secret', roomId, token);
    channelRef.current = channel;
    channel.on('broadcast', { event: `state:${playerIdRef.current}` }, ({ payload }) => {
      if (payload && typeof payload === 'object' && (payload as PhoneState).type === 'secret-phone-state') { setGame(payload as PhoneState); setStatus('connected'); }
    }).subscribe(nextStatus => {
  ...
  const join = (event: FormEvent) => { event.preventDefault(); const clean = name.trim(); const channel = channelRef.current; if (!clean || !channel || status !== 'connected') return; void channel.send({ type:'broadcast', event:'join', payload:{ type:'join', playerId:playerIdRef.current, name:clean.slice(0,18) } satisfies ClientMessage }); setJoined(true); };
  const vote = (targetId: string) => { const channel = channelRef.current; if (!channel || status !== 'connected' || game?.votedFor) return; void channel.send({ type:'broadcast', event:'vote', payload:{ type:'vote', playerId:playerIdRef.current, targetId } satisfies ClientMessage }); };
