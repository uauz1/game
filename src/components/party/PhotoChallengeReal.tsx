...
export default function PhotoChallengeReal({ onHome }: GameProps) {
  ...
  useEffect(()=>{if(phase==='play'&&card&&!revealed&&!timedOut){publishMultiplayerChallenge({gameId:'photo',roundKey:card.id,answers:[card.answer],points});return()=>clearMultiplayerChallenge('photo');}clearMultiplayerChallenge('photo');},[card,phase,points,revealed,timedOut]);
  useEffect(()=>{const receive=(event:Event)=>{const detail=(event as CustomEvent<{gameId?:string;team?:number;points?:number}>).detail;if(detail?.gameId!=='photo'||phase!=='play'||revealed||timedOut)return;const team=detail.team===1?1:0;const awarded=typeof detail.points==='number'?detail.points:points;setTeams(value=>value.map((item,index)=>index===team?{...item,score:item.score+awarded}:item));setRevealed(true);};window.addEventListener('qaddha:multiplayer-team-score',receive);return()=>window.removeEventListener('qaddha:multiplayer-team-score',receive);},[phase,points,revealed,timedOut]);
