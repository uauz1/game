...
if (exists(durableOnlineFile)) {
  ...
  const currentOnlineChecks = [
    ...
    [read('src/utils/onlineEmbedBridge.ts').includes("emit('submit'") && read('src/utils/onlineEmbedBridge.ts').includes('requestSubmit()') && durableOnline.includes("'submit'"), 'Online form submissions synchronize across devices'],
    ...
    [read('src/utils/onlineEmbedBridge.ts').includes('authoritySeq?: number') && durableOnline.includes('gameActionSeq') && durableOnline.includes('authorizeGameAction') && durableOnline.includes('Math.max(authoritySeqRef.current'), 'Online gameplay actions are ordered by host authority for deterministic replay'],
    ...
    [durableOnline.includes('roomBroadcastKey') && durableOnline.includes("type: 'game-state'") && durableOnline.includes('actionLogRef') && durableOnline.includes('gameStateRef'), 'High-frequency gameplay traffic is separated from full room snapshots to reduce lag'],
    [durableOnline.includes('window.setTimeout(() => {') && durableOnline.includes('gameActions: actionLogRef.current') && durableOnline.includes('420'), 'Gameplay persistence is batched instead of re-rendering the room on every action'],
    [read('src/utils/onlineEmbedBridge.ts').includes('action.gameId !== gameId') && durableOnline.includes('action.gameId !== room.gameId') && durableOnline.includes('action.gameId !== activeGameId'), 'Stale online actions are scoped to the active game'],
    ...
    [durableOnline.includes("!['auction','order','memory','missing','intruder'].includes(room.gameId)") && durableOnline.includes("['letters','who','pressure','intruder'].includes(room.gameId)"), 'Online lobby hides settings that do not apply to the selected game'],
    ...
    [durableOnline.includes('actionPersistTimerRef') && durableOnline.includes('420') && durableOnline.includes('gameActions: actionLogRef.current'), 'Online action persistence is batched to reduce realtime lag'],
